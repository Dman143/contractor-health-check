...
export type HealthTrack = 'Delivery & Workmanship' | 'Client Experience & Communication' | 'Commercial Control' | 'Financial Control' | 'Demand & Positioning' | 'Capacity & Direction';
export type RawAnswer = string | number | boolean | null;
export type Question = { id: string; track: HealthTrack; prompt: string; type: 'SCALE' | 'SINGLE_SELECT' | 'TEXT'; scored: boolean; conditional?: 'HAS_TEAM'; options: readonly { value: RawAnswer; label: string; scoreValue?: number }[] };
