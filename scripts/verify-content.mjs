...
const data=readFileSync(new URL('../src/data.ts',import.meta.url),'utf8'); const app=readFileSync(new URL('../src/App.tsx',import.meta.url),'utf8'); const styles=readFileSync(new URL('../src/styles.css',import.meta.url),'utf8'); const assessment=readFileSync(new URL('../src/assessment.ts',import.meta.url),'utf8'); const server=readFileSync(new URL('../server/index.mjs',import.meta.url),'utf8');
...
if(!existsSync(new URL('../public/tradebuilt-logo-cream.png',import.meta.url))) throw new Error('Missing approved TradeBuilt cream-background logo.');
...
for(const text of ['Thanks - got it.','I can personally review your answers','I will read your message and reply directly. In the meantime, the free Health Check can help you clarify what needs attention first.','Download the Free Quick Guide','href="https://www.tradebuilt.pro/"','src="/tradebuilt-logo-cream.png"']) if(!app.includes(text)) throw new Error(`Missing personal-review confirmation content: ${text}`);
if(!styles.includes('.completion-full-stop { color: #F5662D; }')) throw new Error('Completion full stop does not use the approved TradeBuilt orange.');
