...
export const createLocalConsultingInsights = ({ leadProfile, results, assessmentAnswers = [] }) => {
  if (assessmentAnswers.length === 25 && assessmentAnswers.every(({ score }) => score === 5)) {
    ...
    return {
      ...
      finalRecommendation: 'Retain the 100/100 assessment score and Elite Contractor Performance Rating as self-reported. Validate both through records, team observation, customer outcomes, and an independent review before using them as objective claims.',
    };
  }
